...
class UserDB implements InterfaceUserDB{
	...
	function getOrganization($usercode = '') {
		$_deptstruct = array ();
	 	$sql = "SELECT d.code,d.name,d.leader,d.d_level 
                FROM dbo.employee e 
                INNER JOIN dbo.department_flat f ON e.deptcode = f.code
                INNER JOIN dbo.department d ON f.p_code = d.code
                WHERE salarysn = '$usercode'
                ORDER BY d.d_level desc 
                 ";
        $result = DBCommon::query ( $sql );

        while ( $row = DBCommon::fetch_array ( $result, MSSQL_ASSOC ) ) {
            $deptarray = array ();
            $deptarray ['id'] = $row ['code'];
            $deptarray ['code'] =  $row ['code'] ;
            $deptarray ['name'] =  wf_iconvutf($row ['name']) ;
            $deptarray ['leader'] = $row ['leader'];
            $deptarray ['level'] = intval( $row ['d_level'] );
            $_deptstruct ['dept' . $row ['code']] = $deptarray;
        }
