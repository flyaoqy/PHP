...
abstract class AbstractOrgRule extends WorkflowBase {
    ...
    public function __construct() {
        ...
        if (!defined("ORGANIZATION_RESULT")) {
            define ("ORGANIZATION_RESULT", '<result step="#stepid#" type="org"><roles><role type="11" typeName="部门领导审批"><usercode>#leader#</usercode></role></roles></result>');
        }
        //组织结构xml模板
    ...
    public function specialRule() {
        ...
        if (count($skiparray) == 0) {
            wf_debug('均不满足条件，满足规则===系统默认规则逐级审批');
            $skipLevel = $this->skipByLevel($orgarray, '', '');
            $skiparray = $skipLevel;
        }
        return $skiparray;
    }
