...
class XmlUtil{
     ...
     *
     */
    public static function getInitialId($wf_xml='') {
        $id = "500";//起始id默认为 1
        if($wf_xml == ''){
            $commandContext = CommandContext::getInstance();
            $wf_uid = $commandContext->getWfuid();
        ...
        $initial_node = $wf_xml->getElementsByTagName ( 'initial-actions' );
        if ($initial_node->length > 0) {
            $node = $initial_node->item ( 0 );
     ...
     *
     */
    public static function getEndId($wf_xml='') {
        $id = "1";//起始id默认为 1
        if($wf_xml == ''){
            $commandContext = CommandContext::getInstance();
            $wf_uid = $commandContext->getWfuid();
        ...
        $end_node = $wf_xml->getElementsByTagName ( 'node' );
        if ($end_node->length > 0) {
            $node = $end_node->item ( 0 );
            $id = $node->getAttribute ( "id" );
        }
        return $id;
    }
