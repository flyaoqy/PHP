...
require_once 'DBcommonMysqli.php';
...
class DBCommon{
	...
	public static function get_db_driver($params) {
		...
		}elseif($params['dbdriver']=='mysqli'){
			$db_driver = new DBcommonMysqli($params);
		}elseif($params['dbdriver']=='mssql'){
			$db_driver = new DBcommonMssql($params);
		}
		return $db_driver;
	}
	/**
	...
	public static function get_self_driver($params = 'default') {
		if(!self::$db_driver){
			return self::init_connect($params);
		}else{
			return self::$db_driver;
		}
	}
	...
	public static function end_tran($flag = true) {
		if(!self::$disable_tran){//是否禁用事务
			...
			if (self::$sql_flag && $flag) {
                return $db_driver->commit_tran();
			} else {
                return $db_driver->rollback_tran();
			}
		}else{
            return false;
        }
