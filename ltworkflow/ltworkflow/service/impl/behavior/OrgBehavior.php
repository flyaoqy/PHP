...
class OrgBehavior {
    ...
    public function getResult($wf_result, $orgarray) {
        ...
        $nextStepid = $commandContext->getNextStepid();
        $wf_node =  XmlUtil::getElementById( $wf_id );
        $wf_node_type =  $wf_node->getAttribute ( "type" );//当前节点类型
        if($wf_node_type == "org"){
            $stepid = $wf_id;
        }else{
            $stepid = $nextStepid;
        ...
        $orgresult = str_replace("#stepid#", $stepid, $orgresult);
    ...
    public function isOrgApprove() {
        ...
        $wf_node =  XmlUtil::getElementById( $wf_id );
        $wf_node_type =  $wf_node->getAttribute ( "type" );//当前节点类型
        $orglist = $this->__doOrganization($wf_etuid, $wf_node_type);
    ...
    //组织结构处理
    private function __doOrganization($etuid, $wf_node_type) {
        ...
        */
        if ($wf_node_type != 'org' && count(array_diff_key($orgarray_all, $skipstruct)) == 0) {
